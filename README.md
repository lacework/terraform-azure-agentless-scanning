...
| <a name="input_subscriptions_list"></a> [subscriptions\_list](#input\_subscriptions\_list) | List of subscriptions to be scanned or excluded. Prefix subscription IDs with `'-'` to exclude it from scanning. Either provide subscription IDs to include OR subscription IDs to exclude, but not both. Set only for global resource. Set only when `integration_level = 'TENANT'`. | `set(string)` | `[]` | no |
